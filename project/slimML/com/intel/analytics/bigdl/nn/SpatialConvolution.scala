...
class SpatialConvolution[T: ClassTag](
  ...
  override def accGradParameters(input: Tensor[T], gradOutput: Tensor[T]): Unit = {
    ...
    if (input.nDimension() == 3) {
      ...
      if(gradWeightMMInBatch.size(1) != batchSize){
        gradWeightMMInBatch = Tensor[T]().resize(getGradWeightMMInBatchShape(batchSize)).zero()
      }
      if(withBias && gradientBiasMT.nElement() == 0) {
        gradientBiasMT.resize(Array(batchSize, nOutputPlane))
      }
      if(withBias && gradientBiasMT.size(1) != batchSize){
        gradientBiasMT.resize(Array(batchSize, nOutputPlane)).zero()
      }
